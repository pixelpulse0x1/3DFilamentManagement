...
function loadImages() {
    fetch('/api/images')
                    ...
                    <div class="image-card-info">
                        ...
                        <div class="image-card-meta">
                            <span class="image-ref-count ${img.ref_count > 0 ? 'has-refs' : ''}">
                                <i class="fas fa-link"></i> ${img.ref_count} 卷耗材引用
                            </span>
                        </div>
                    </div>
                    <div class="image-card-actions">
                        <button class="btn btn-outline edit-img-btn" data-id="${img.id}" data-name="${img.name}" data-ref="${img.ref_count}">
                            <i class="fas fa-edit"></i>
                        </button>
                        <button class="btn btn-danger del-img-btn" data-id="${img.id}" data-name="${img.name}" data-ref="${img.ref_count}">
        ...
        .catch(err => console.error('加载实物图失败:', err));
}
...
function openAddImage() {
    ...
    document.getElementById('imageModalTitle').textContent = '上传实物图';
...
function openEditImage(id, name, refCount) {
    if (refCount > 0) {
        ...
        document.getElementById('confirmRiskMessage').innerHTML =
            `该实物图「<strong>${name}</strong>」当前正被 <strong>${refCount}</strong> 卷耗材使用，修改或替换图片将同步影响这些耗材的显示。`;
...
function _doOpenEdit(id, name) {
    ...
    document.getElementById('imageModalTitle').textContent = '编辑实物图';
...
function saveImage() {
    ...
    if (!name) { showImageMsg('请输入图片名称', 'error'); return; }
    ...
    if (fileInput.files.length > 0) {
        ...
        showImageMsg('请选择要上传的文件', 'error'); return;
    }
    ...
    btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> 保存中...'; btn.disabled = true;
    ...
    fetch(url, { method, body: formData })
        ...
        .then(d => {
            if (d.status === 'success') {
                ...
                showImageMsg(d.error || '保存失败', 'error');
            }
        })
        .catch(err => showImageMsg('保存失败: ' + err.message, 'error'))
        .finally(() => { btn.innerHTML = '<i class="fas fa-save"></i> 保存'; btn.disabled = false; });
}

// ─── Delete with Anti-Error ───
...
function deleteImage(id, name, refCount) {
    if (refCount > 0) {
        ...
        pendingImageId = id;
        document.getElementById('confirmRiskMessage').innerHTML =
            `该实物图「<strong>${name}</strong>」当前正被 <strong>${refCount}</strong> 卷耗材使用，执行删除将同步影响这些耗材的显示。`;
        document.getElementById('confirmRiskModal').style.display = 'flex';
        return;
    }
    _doDelete(id);
...
function executePendingAction() {
    ...
    if (pendingAction === 'delete') {
        ...
        // Get the name from the stored button dataset
