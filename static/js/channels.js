    ...
    fetch('/api/channels')
        ...
        .then(data => {
            ...
            if (!Array.isArray(data) || data.length === 0) {
                tbody.innerHTML = '<tr><td colspan="4"><div class="empty-state"><i class="fas fa-store"></i><p>暂无渠道，点击上方按钮添加</p></div></td></tr>';
                return;
            }
            tbody.innerHTML = '';
            data.forEach(ch => {
...
function saveChannel() {
    ...
    fetch(url, {
        ...
        .then(r => r.json())
        .then(d => { if (d.status === 'success') { closeModal(); loadChannels(); } else alert(d.error || '保存失败'); })
...
function deleteChannel(id, name) {
    ...
    fetch('/api/channels/' + id, { method: 'DELETE' })
        .then(r => r.json())
        .then(d => { if (d.status === 'success') loadChannels(); else alert(d.error || '删除失败'); })
